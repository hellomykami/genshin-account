...
namespace GenshinAccount
{
    public partial class FormMain : Form
    {
        ...
        private void StartGameNomal()
        {
            ...
            startInfo.FileName = Path.Combine(txtPath.Text, "Genshin Impact Game", "GenshinImpact.exe");
        ...
        private void StartGame()
        {
            ...
            else
            {
                try
                {
                    ...
                    var result = DllUtils.CreateProcessAsUser(hToken, Path.Combine(txtPath.Text, "Genshin Impact Game", "GenshinImpact.exe").ToString(),
        ...
        private void Switch(string name, bool autoRestart)
        {
            ...
            if (chkSkipTips.Checked || MessageBox.Show($"是否要切换为[{name}]", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    var pros = Process.GetProcessesByName("GenshinImpact");
                    if (pros.Any())
                    {
                        pros[0].Kill();
                    }
        ...
        private void btnChoosePath_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!string.IsNullOrEmpty(dialog.SelectedPath) && File.Exists(Path.Combine(dialog.SelectedPath, "GenshinImpact.exe")))
                {
                    dialog.SelectedPath = dialog.SelectedPath.Replace("Genshin Impact Game", "");
                }
                if (string.IsNullOrEmpty(dialog.SelectedPath) || !File.Exists(Path.Combine(dialog.SelectedPath, "Genshin Impact Game", "GenshinImpact.exe")))
                {
